class Message < ActiveRecord::Base
  after_create {
    |record|
    ## Set the root_id for non recurring messages
    ## recurring message's are not message threads. They are more
    ## like email templates which are used at used to send messages in future.
  ...
  def cached_recipients
    ...
    if( rcpt_list.nil? || rcpt_list.blank? )
      rcpt_list = self.recipients.collect(&:name).join(",")
...
#  lft                    :integer(4)
#  root_id                :integer(4)
#  cached_recipients_list :string(255)
#  created_at             :datetime
#  updated_at             :datetime
#  rgt                    :integer(4)
#  msg_type               :string(255)
