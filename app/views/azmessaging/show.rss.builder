...
xml.rss "version" => "2.0", "xmlns:dc" => "http://purl.org/dc/elements/1.1/" do
  xml.channel do
    xml.title       "AzMessaging Messages thread #{@root_id} for " + current_user.login.to_s
    xml.link        url_for :only_path => false, :controller => 'dashboard/messages'
    xml.description "AzMessaging Message thread #{@root_id} for #" + current_user.login.to_s
